...
class WordPipeline:
    ...
    async def process_word(self, job: Job) -> None:
        ...
        try:
            async with aclosing(completion):
                async for delta in completion:
                    ...
                    entry.text = visible
                    now = self._clock()
                    if visible != last_published and (
                        last_update_at is None or now - last_update_at >= UPDATE_INTERVAL_SECONDS
                    ):
        ...
        if final != last_published:
            await self._publish_update(entry)
        entry.status = "done"
        await self.events.publish("done", {"entry_id": entry.entry_id, "text": final})
        self._trim_history()
