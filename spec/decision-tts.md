...
- Model downloads are config-driven: only engines and voices
  ...
  downloaded.
- **The two most recently spoken Piper voices stay loaded**, and the
  startup pass loads that many of the configured ones so the first word
  after a restart does not pay for it. Measured on the deployment host:
  ...
  that on nearly every word, because the reader alternates between
  languages within a session. Two is where the memory limits in
...
- **The prefix is also the accent.** It is what the recordings of a
  ...
  has a few dozen word recordings in all, and Serbian's thousand-odd
  `Sr-` files are transliterated place names, so both are spoken by the
  engines alone; such a row gains a prefix when somebody measures one.
